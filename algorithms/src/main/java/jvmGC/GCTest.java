...
public class GCTest {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 1; i <= 12; i++){
            if(i==11) {
                System.out.println(new Date() + "--caches准备添加第11次,old区内存不够，开始full GC 前先执行minor GC 第"+5+"次,FGC 第1次(触发条件：【MinorGC后存活的对象超过了老年代剩余空间】)");
            }
            caches.add(new byte[3 * UNIT_MB]);
            if(i%2==0 && i!=10) {
                System.out.println(new Date() + "--caches添加第"+i+"次后，eden + survivor 的内存不够，开始minor GC 第"+count+"次");
                count++;
            }else {
                System.out.println(new Date() + "--caches添加第"+i+"次");
            }
        }
        ...
        caches.remove(0);
        System.err.println("--FGC开始 第2次（触发条件：晋升到老年代的大小超过了老年代剩余大小）");
        caches.add(new byte[3 * UNIT_MB]);
        System.err.println("本次FGC，移植了Young区的一部分到Old区，导致Young区还有3M左右");
        for (int i = 0; i < 8; i++){
            // 这里是为了下次FGC后，直接减少老年代的内存大小，从而正常YGC
            caches.remove(0);
        }
        System.err.println("--FGC开始 第3次(触发条件:同上)");
        caches.add(new byte[3 * UNIT_MB]);
